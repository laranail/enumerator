...
it('omits the integrity attribute when the config value is empty', function (): void {
    config()->set('laranail.enumerator.alpine.integrity', '');
    $html = Blade::render('<x-laranail-enumerator::alpine-loader />');
...
it('substitutes the version placeholder in the CDN URL pattern', function (): void {
    config()->set('laranail.enumerator.alpine.version', '3.14.0');
    config()->set('laranail.enumerator.alpine.cdn_url', 'https://example.test/alpine@{version}.js');
    config()->set('laranail.enumerator.alpine.integrity', ''); // skip SRI for the alternate CDN
